...
if [ $ip == "ipv6" ] ; then
  iperf -c $remote_host -V -u -t 30 -r -y c | sed '1d' >> iperf_udp_test/iperf_udp_$connection_method.csv 
else
  iperf -c $remote_host -u -t 30 -r -y c | sed '1d' >> iperf_udp_test/iperf_udp_$connection_method.csv  ## delete -V iff the host is not ipv6
fi
...
# Add section for plotting graphs
